...
class ExtractText:
    ...
    def dic_to_html(
        ...
        return (
            '<!DOCTYPE html>'
            f'<head><meta charset="UTF-8">{style}</head>'
            '<body><center>'
            f'{banner}{words_log}<br><br>'
            f'<table>{html}</table>'
            '</center></body>'
        )
...
def main(ext: str = 'pdf'):
    ...
    fname = 'my.pdf'
    if ext == 'srt':
        fulltext = extract_text.read_srt(fname)
        (words_list,
            not_added_count,
            blacklist_words,
            not_word_count,
            duplicates_count) = extract_text.clean_words(fulltext)
    elif ext == 'pdf':
        # fulltext = extract_text.read_pdf(fname, 707)
        fulltext = extract_text.read_pdf(fname)
        print(fulltext)
        (
            words_list,
            not_added_count,
        ...
        fulltext = extract_text.read_txt(fname)
        (
            words_list,
            not_added_count,
            blacklist_words,
